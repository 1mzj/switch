...
if 'dist' in current_path:
    ...
    model_path = os.path.join(app01_path, "Pre_model/dlq/", "cnn_model.pth")
    scaler_X_path = os.path.join(app01_path, "Pre_model/dlq/", 'scaler_X.pkl')
    scaler_y_path = os.path.join(app01_path, "Pre_model/dlq/", 'scaler_y.pkl')
else:
    model_path = os.path.join("main_app/Pre_model/dlq/", "cnn_model.pth")
    scaler_X_path = os.path.join("main_app/Pre_model/dlq/", 'scaler_X.pkl')
    scaler_y_path = os.path.join("main_app/Pre_model/dlq/", 'scaler_y.pkl')
...
# 预测函数
def predict_duanluqi_res(data):
    """
    预测断路器电阻
    :param data: list or numpy array, 格式 [电流, 环境温度, 环境湿度, 风速, 辐射强度,断路器温度]
