...
if 'dist' in current_path:
    ...
    model_path = os.path.join(app01_path, "Pre_model/glkg/", "cnn_model_tep.pth")
    scaler_X_path = os.path.join(app01_path, "Pre_model/glkg/", 'scaler_X_tep.pkl')
    scaler_y_path = os.path.join(app01_path, "Pre_model/glkg/", 'scaler_y_tep.pkl')
else:
    model_path = os.path.join("main_app/Pre_model/glkg/", "cnn_model_tep.pth")
    scaler_X_path = os.path.join("main_app/Pre_model/glkg/", 'scaler_X_tep.pkl')
    scaler_y_path = os.path.join("main_app/Pre_model/glkg/", 'scaler_y_tep.pkl')
...
# 预测函数
def predict_gelikaiguan_tep(data):
    """
    预测隔离开关温度
    :param data: list or numpy array, 格式 [电流, 环境温度, 环境湿度, 风速, 辐射强度, 隔离开关电阻]
