...
if 'dist' in current_path:
    ...
    model_path = os.path.join(app01_path, "Pre_model/hgq/", "cnn_model_tep.pth")
    scaler_X_path = os.path.join(app01_path, "Pre_model/hgq/", 'scaler_X_tep.pkl')
    scaler_y_path = os.path.join(app01_path, "Pre_model/hgq/", 'scaler_y_tep.pkl')
else:
    model_path = os.path.join("main_app/Pre_model/hgq/", "cnn_model_tep.pth")
    scaler_X_path = os.path.join("main_app/Pre_model/hgq/", 'scaler_X_tep.pkl')
    scaler_y_path = os.path.join("main_app/Pre_model/hgq/", 'scaler_y_tep.pkl')
...
# 预测函数
def predict_huganqi_tep(data):
    """
    预测互感器温度
    :param data: list or numpy array, 格式 [电流, 环境温度, 环境湿度, 风速, 辐射强度, 互感器电阻]
